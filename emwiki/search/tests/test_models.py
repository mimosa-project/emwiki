...
class TheoremTest(TestCase):
    ...
    def test_register_theorem(self):
        ...
        self.assertEqual(self.number_of_results, theorem_count_label, theorem_count_text)
...
class HistoryItemTest(TestCase):
    ...
    def test_register_history_item(self):
        ...
        self.assertEqual(self.number_of_results, history_item_count_history, history_item_count_theorem)
        self.assertEqual(self.number_of_results, history_item_count_relevance)
        self.assertEqual(self.number_of_results, history_item_count_favorite, history_item_count_click)

    def test_update_search_results_id(self):
        # 検索結果の中から定理のテーブルに登録されていない定理をテーブルに保存
        Theorem.register_theorem(self.search_results)
        # クエリをデータベースに登録
        self.history = History.objects.create(query=self.query_text)
        ...
        HistoryItem.register_history_item(self.search_results, self.history)
        # 検索結果のIDをデータベースから取得
        self.search_results = HistoryItem.update_search_results_id(self.search_results, self.history)
        ...
        for i in range(self.number_of_results):
            self.assertGreaterEqual(db_id_list[i], search_results_order_by_label[i]['id'])
