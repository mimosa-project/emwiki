...
class TheoremTest(TestCase):
    ...
    def test_register_theorem(self):
        ...
        theorem_count_label = Theorem.objects.filter(
            label__isnull=False).count()
        theorem_count_text = Theorem.objects.filter(
            theorem__isnull=False).count()
...
class HistoryItemTest(TestCase):
    ...
    def test_register_history_item(self):
        ...
        history_item_count_history = HistoryItem.objects.filter(
            history__isnull=False).count()
        history_item_count_theorem = HistoryItem.objects.filter(
            theorem__isnull=False).count()
        history_item_count_relevance = HistoryItem.objects.filter(
            relevance__isnull=False).count()
        history_item_count_favorite = HistoryItem.objects.filter(
            favorite__isnull=False).count()
        history_item_count_click = HistoryItem.objects.filter(
            click__isnull=False).count()
    ...
    def test_collect_history_item_id(self):
        ...
        db_id_list = HistoryItem.objects.values_list(
            'id', flat=True).order_by('theorem__label')
        search_results_order_by_label = sorted(
            self.search_results, key=lambda x: x['label'])
        for i in range(self.number_of_results):
            self.assertEqual(
                db_id_list[i], search_results_order_by_label[i]['id'])
