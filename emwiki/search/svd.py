...
def singular_value_analysis(input_txt):
    ...
    dictionary.save(os.path.join(DATA_FOR_SEARCH_DIR, 'search_dictionary.dict'))
    ...
    tfidf_model.save(os.path.join(DATA_FOR_SEARCH_DIR, 'tfidf.model'))
    corpora.MmCorpus.serialize(os.path.join(DATA_FOR_SEARCH_DIR, 'tfidf_corpus.mm'), tfidf_corpus)
    ...
    lsi_model.save(os.path.join(DATA_FOR_SEARCH_DIR, 'lsi_topics300.model'))

    # 類似度を求めるためのindexの作成
    index = similarities.MatrixSimilarity(lsi_corpus)
    index.save(os.path.join(DATA_FOR_SEARCH_DIR, 'lsi_index.index'))
