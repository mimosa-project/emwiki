...
function makeRequest(obj, url, tooltip) {
  ...
  if (rrCache[url] != null) {
    ...
    http_request.onreadystatechange = function() {
      insertRequest(obj, http_request, url, tooltip);
    };
...
function insertRequest(obj, http_request, url, tooltip) {
  ...
  if (respText != null) {
    ...
    if (http_request != null) {
      rrCache[url] = respText;
    }
    if (tooltip != null) {
      obj.innerHTML = respText;
    } else {
      let ndiv = obj;
      while (ndiv.nodeName != 'SPAN') {
        ndiv = ndiv.nextSibling;
      }
...
var tooltip = function() {
  ...
  const endalpha = 95;
  let alpha = 0;
  let tt; let t; let c; let b; let h;
  ...
  return {
    show: function(how, v, w) {
      ...
      if (how == 'url') {
        ...
        if ((how == 'hs') || (how == 'hs2')) {
          tt.innerHTML = v.nextSibling.innerHTML;
        } else {
          if (how == 'txt') {
            tt.innerHTML = v;
          } else {
...
function rs(ref) {
  tooltip.show('url', mizhtm + 'refs/' + ref);
}
function rh() {
  tooltip.hide();
}
