...
class MizTextConverterTest(TestCase):
    ...
    def test_extract_comments(self):
        ...
        for comment in comments:
            self.assertIn(comment['block'], [comment['block']
                          for comment in self.comments])
            self.assertIn(comment['block_order'], [
                          comment['block_order'] for comment in self.comments])
            self.assertIn(comment['text'], [comment['text']
                          for comment in self.comments])

    def test_add_comment_header(self):
        for comment in self.comments:
            comment_text = self.miztextconverter.add_comment_header(
                comment['text'])
    ...
    def test_restorebility(self):
        ...
        for i in range(10):
            comments = self.miztextconverter.extract_comments(
                processed_commented_text)
            processed_raw_text = self.miztextconverter.remove_comments(
                processed_commented_text)
            processed_commented_text = self.miztextconverter.embed_comments(
                processed_raw_text, comments)
